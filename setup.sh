...
[ ! -d $DOCKER_GOINFRE ] && echo "⭐️ $(tput setaf 1)------- Setup docker --------$(tput sgr 0) ⭐️" && brew install docker docker-machine && docker-machine rm default >/dev/null 2>&1 && mv $DOCKER_HOME $DOCKER_GOINFRE && ln -s $DOCKER_GOINFRE $DOCKER_HOME
...
[ ! -d $MINIKUBE_GOINFRE ] && echo "⭐️ $(tput setaf 1)------- Setup k8s + minikube ---------$(tput sgr 0) ⭐️" && brew install minikube && mv $MINIKUBE_HOME $HOME/goinfre/ && ln -s $MINIKUBE_GOINFRE $MINIKUBE_HOME
